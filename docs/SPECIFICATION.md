...
Вверху — горизонтальная навигация: **Курсы** | **Статистика** | **Импорт** | **Инфо** | **Генерация промта**. По умолчанию открыта вкладка **Курсы**.
...
Поле **«Описание курса»** и кнопка **«Копировать в буфер обмена»**. В clipboard — промт для внешнего LLM: описание автора, инструкции (курс **на русском языке**; в **каждом** модуле шаги `theory`, `svg`, `quiz`; ответ — только JSON **import-DTO** без UUID и `createdAt`) и полный текст bundled [course-import.schema.json](./schemas/course-import.schema.json).
