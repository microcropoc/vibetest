...
|-----|-----------|--------|
| `javascript` | после await: по `resultMode` (default `return`) — return и/или args после `structure` serialize; при `rejects` — только reason. Для сравниваемых значений: **JSON-совместимо** (кроме `resultMode: "args"|"both"`, где return не сравнивается или сравнивается отдельно). **Не-JSON** в сравниваемом return: `undefined`, `function`, `symbol`, `bigint`, не-plain объекты. **Числа:** `Object.is` (`NaN`, ±0); `Infinity` допустимы. Recursive equal. | `setup`/`reset`; `{ "args", "calls"?, "rejects"?, "expectInvocations"?, "advanceMs"?, "flushMicrotasks"? }`; `functionName`; опционально `resultMode`, `structure`; `timeoutMs`; в `setup` — `registerSpy(name, fn)` (раннер также инжектирует helper) |
