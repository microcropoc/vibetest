...
Вверху — горизонтальная навигация: **Курсы** | **Статистика** | **Импорт** | **Инфо** | **Генерация промта**. По умолчанию открыта вкладка **Курсы**. В **footer** shell — версия сборки: короткий hash git и subject коммита на момент `ng build` (для проверки деплоя).
