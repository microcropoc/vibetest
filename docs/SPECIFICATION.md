...
Проверка в Web Worker. Обязательное **`timeoutMs`** (100–30000): один deadline на **весь** шаг практики (javascriptInit/sqliteInit/regexInit + все `tests[]`, не per-case); кооперативная проверка, при зависании — `Worker.terminate()`. У JavaScript cap **checker** 250 ms — только на thenable-return (отдельный от `deadlineMs` кейса); sync hang в checker — общий `timeoutMs` / terminate. Курсы **доверенные**; Worker защищает от зависания, не от произвольного кода в origin.
